...
# Idagdag ito sa itaas ng 'ai_spotter_plane.gd' kasama ng iba pang exports:
...
@export var spread_deg: float = 1.0
...
@onready var plane_mesh =  $BiSpotterV1
...
func _start_crash() -> void:
	...
	# 1. Dagdag Puntos (Score) sa GameManager
	if GameManager.has_method("add_points"):
		GameManager.add_points(score_value)
		
	# 2. Dagdag XP (Kung may LevelManager autoload)
	if is_instance_valid(get_node_or_null("/root/LevelManager")):
		var level_manager = get_node("/root/LevelManager")
		if level_manager.has_method("add_xp"):
			level_manager.add_xp(xp_reward)
			
	# 3. Dagdag Credits gamit ang CurrencyManager Autoload (BAGO)
	if is_instance_valid(get_node_or_null("/root/CurrencyManager")):
		var currency_manager = get_node("/root/CurrencyManager")
		currency_manager.add_currency("credit", credit_reward)
		print("[REWARD] Plane shot down! Earned +", credit_reward, " credits.")
		
	# 4. Magpalitaw ng Floating Reward Text sa screen (BAGO)
