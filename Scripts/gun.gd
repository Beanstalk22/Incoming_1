...
@onready var gun_hp: ProgressBar = $gun/CanvasLayer/GunHp
...
@export_group("Health")
@export var max_health : int = 100 # Toned this down slightly for readability 
...
func _ready() -> void:
	...
	# --- ADD THIS FIX HERE ---
	# Siguraduhing nakatago ang UI kapag nag-spawn ang gun
	if gun_hp:
		gun_hp.visible = false
	if ammo_label:
...
func enter_gun(player: CharacterBody3D) -> void:
	...
	# KEY CHANGE: Tinawag natin ang function para itago ang wrench at binoculars
	if player_ref.has_method("set_mounted"):
		player_ref.set_mounted(true)
...
func exit_gun() -> void:
	...
	if gun_hp: gun_hp.visible = false
	if player_ref: 
		is_controlled = false 
		
		# KEY CHANGE: Ibalik ang visibility ng gamit ng player bago siya tanggalin sa gun reference
		if player_ref.has_method("set_mounted"):
			player_ref.set_mounted(false)
...
func fire_weapon() -> void:
	...
	if projectile_scene == null or current_ammo <= 0: return
	
	current_ammo -= 1
	update_ammo_ui() # <--- ADD THIS HERE
...
func repair(amount: int) -> void:
	...
	if current_health > max_health:
		current_health = max_health
	print("Gun repaired! Current health: ", current_health)
	
func update_health_ui() -> void:
	if gun_hp:
		gun_hp.max_value = max_health
		gun_hp.value = current_health
